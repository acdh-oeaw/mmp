...
from topics.models import ModelingProcess, TextTopicRelation, Topic
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        params = {
            ...
            "dict_filter_params": {"no_below": 2, "no_above": 0.5},
        }
        ...
        for i, x in enumerate(topics):
            Topic.objects.create(
                topic_index=i, process=model_process, **top_to_topic_object(x)
            )
        for i, x in tqdm(
            enumerate([model.get_document_topics(item) for item in corpus]),
            total=len(df),
        ):
